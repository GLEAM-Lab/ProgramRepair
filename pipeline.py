...
def parse_openreview_venue(source_meta):
    ...
    支持分页获取完整列表，包括所有类型（Oral, Spotlight, Poster）
    """
    year = source_meta.get("year")
    venue_name = source_meta.get("venue_name", "ICLR")  # e.g., "ICLR 2025"
    
    # OpenReview的venue可能有多个变体：
    # "ICLR 2025", "ICLR 2025 Poster", "ICLR 2025 Spotlight", "ICLR 2025 Oral"
    venue_patterns = [
        venue_name,
        f"{venue_name} Poster",
    ...
    for venue_pattern in venue_patterns:
        ...
        while True:
            # 构建API URL
            api_url = f'https://api2.openreview.net/notes?content.venue={venue_pattern.replace(" ", "+")}&select=id,content&limit={limit}&offset={offset}'
    ...
    for note in notes:
        ...
        title = title_data.get('value', title_data) if isinstance(title_data, dict) else title_data
        ...
        abstract = abstract_data.get('value', abstract_data) if isinstance(abstract_data, dict) else abstract_data
        if not isinstance(abstract, str):
            abstract = ""
        
        # 提取作者
        authors_data = content.get('authors', {})
        authors = authors_data.get('value', authors_data) if isinstance(authors_data, dict) else authors_data
        ...
        record = {
            "source": f"conf:{venue_name.split()[0]}:{year}",
            "title": title.strip(),
            "year": year,
            "doi": paper_url,
            "venue": venue_name,
