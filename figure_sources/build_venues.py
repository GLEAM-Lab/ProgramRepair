...
def normalize_venue(raw: str) -> str:
    ...
    if "neurips" in lower or "neural information processing systems" in lower:
        return "NeurIPS"
    if lower == "acl" or lower.startswith("acl ") or " acl " in lower or "association for computational linguistics" in lower:
        return "ACL"
    if "aaai" in lower:
        return "AAAI"
    return "Other"
...
def load_counts() -> Counter[str]:
    ...
    selected = [r for r in rows if r["current_reference_decision"].strip().lower() == "include"]
    counts: Counter[str] = Counter(normalize_venue(r["venue"]) for r in selected)
    if len(selected) != 66:
...
def write_counts(counts: Counter[str]) -> None:
    ...
    with OUT_COUNTS.open("w", newline="", encoding="utf-8") as f:
        ...
        for venue, count in sorted(counts.items()):
            if venue not in VENUE_ORDER:
                writer.writerow([venue, count])
...
def draw(counts: Counter[str]) -> None:
    ...
    ax.set_xlabel("Number of selected systems", fontsize=10)
    ax.set_title("Venue families of selected works (n = 66)", fontsize=12, fontweight="bold", pad=12)

    for i, value in enumerate(values):
        ax.text(value + 0.25, i, str(value), va="center", ha="left", fontsize=9.5, color="#1f2933")

    ax.spines[["top", "right", "left"]].set_visible(False)
    ax.spines["bottom"].set_color("#C6CDD5")
    ax.tick_params(axis="y", length=0, labelsize=9.5)
    ax.tick_params(axis="x", colors="#586574", labelsize=9)
